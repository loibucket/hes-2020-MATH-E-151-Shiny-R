...
#The user interface
header <- dashboardHeader(title = "Fitting a polynomial - Loi C",
                          titleWidth = 800)
...
body <- dashboardBody(
  fluidRow(stylesheet,
    column(width=4,
      ...
      actionBttn("btnHubb", "S&P 500 prices in the last 5 days, up to 11/16/2020"),
      actionBttn("btnCOVID", "S&P 500 prices in the last 10 days, up to 11/16/2020")
...
server <- function(session, input, output) {
  ...
  observeEvent(input$btnHubb, {
    ...
    b = 5
    x = 1:b
    y = sp$Close[(length(sp$Date)-(b-1)):length(sp$Date)]
    output$formula <- NULL
    coeff <- fit.makePoly(x,y)
    output$graph <- renderPlot({
      curve(fit.eval(coeff,x), from = 1, to = b)
