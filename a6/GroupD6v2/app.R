...
ui <- dashboardPage(
    skin="green",
    dashboardHeader(title = "Group D6 by Loi (v2)", titleWidth=600),
        ...
        fluidRow(
            column(width=4,
                   box(
                       ...
                       h5("Order 6 elements (rotation)"),
                       controlRow2(
                           c("ctrl123456", "ctrl165432")
                       ),  #agb
                       h5("Order 3 elements, (edge flip)"),
                       controlRow2(
                           c("ctrl135.246", "ctrl153.264")
                       ),  #agb
                       h5("Order 2 elements (vertex flip)"),
                       controlRow4(
                         c("ctrl13.46","ctrl26.35","ctrl15.24","ctrl14.25.36")
                       ),  #agb
                       br(),
                       controlRow3(
                         c("ctrl14.23.56","ctrl12.36.45","ctrl16.25.34")
                       )  #agb
                   ),#box
                   box(
                       width = NULL,
...
#This algorithm could turn out to be inefficient, and in principle it can fail
    ...
    observeEvent(input$btngen,{
      ...
      while(TRUE){
        ...
        if (length(subs) > 12){break}
      }
      subgroup <<- numeric(0)
      for(element in subs){
        k <- which(D6DF[,2] == element)[1]
        subgroup <<- append(subgroup,k)
      }
      mark.subgroup()
      showButtons()
      output$genmsg <-
